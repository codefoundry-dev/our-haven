...
/**
 * Table-routed Kysely fake for the Caregiver Opportunities routes (OH-218). Every
 * `selectFrom(t)` resolves to `tables[t]`'s canned rows; `.where()` / `.orderBy()`
 * are NO-OPS (SQL-level filtering — state/origin/category/schedule_kind — is the
 * real DB's job, so these tests exercise only the in-JS logic: category
 * resolution, the detail visibility rule, recency+distance ranking, the
 * applicant/self-state fold, and the derived quota math).
 */
function makeDb(tables: Record<string, Record<string, unknown>[]> = {}) {
  ...
  return { db: handle as unknown as AppDeps['db'] };
}
