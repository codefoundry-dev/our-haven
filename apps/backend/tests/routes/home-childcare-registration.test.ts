...
describe('GET /v1/providers/me/home-childcare-registration', () => {
  ...
  it('marks homeChildcareBoardSupported=false for a Nanny in a non-slate state', async () => {
    ...
    const app = await buildAppWithRoutes(makeDeps({ db }));
    try {
      const token = await providerToken(NANNY_AK.uid);
      const res = await app.inject({
        method: 'GET',
        url: '/v1/providers/me/home-childcare-registration',
        headers: { authorization: `Bearer ${token}` },
  ...
  it('409s for a Tutor Caregiver', async () => {
    ...
    const app = await buildAppWithRoutes(makeDeps({ db }));
    try {
      const token = await providerToken(TUTOR.uid);
      const res = await app.inject({
        method: 'GET',
        url: '/v1/providers/me/home-childcare-registration',
        headers: { authorization: `Bearer ${token}` },
  ...
  it('409s for a Specialist', async () => {
    const { db } = makeDbStub({ providerByUid: SPECIALIST });
    const app = await buildAppWithRoutes(makeDeps({ db }));
    try {
      const token = await providerToken(SPECIALIST.uid);
      const res = await app.inject({
        method: 'GET',
        url: '/v1/providers/me/home-childcare-registration',
        headers: { authorization: `Bearer ${token}` },
...
describe('GET /v1/admin/providers/:providerId/home-childcare-registration', () => {
  ...
  it('409s when the target Provider is a Specialist', async () => {
    const { db } = makeDbStub({ providerById: SPECIALIST });
    const app = await buildAppWithRoutes(makeDeps({ db }));
    try {
      const token = await adminToken('admin-1');
      const res = await app.inject({
        method: 'GET',
        url: `/v1/admin/providers/${SPECIALIST.id}/home-childcare-registration`,
        headers: { authorization: `Bearer ${token}` },
      });
      expect(res.statusCode).toBe(409);
    } finally {
