...
> The Florida soft-launch posture from ADR-0003 (Miami-Dade marketing concentration, FL-statewide geofence, FL-board-eccentric verification) is **superseded by ADR-0009 (2026-05-26)**. Federal compliance design and vendor choices (Checkr, Stripe Connect Express US, Daily.co US rooms) are unchanged. The platform stack (Firebase Auth + GCP) was re-architected to **Supabase + Fly.io + Vercel** per **ADR-0010 (2026-05-27)**, then to **Supabase (Auth + Postgres + Realtime + Storage + Edge-Functions backend) + Vercel (admin web)** per **ADR-0019 (2026-06-24)**, which drops Fly.io in favour of a single Hono "fat" Edge Function; data-residency remains US-only.
...
### Data residency
All personal data is processed in **US regions**. Vendor settings (Supabase → US-region project, hosting Auth + Postgres + Realtime + Storage + the Edge-Functions backend; Vercel → US-region for the admin surface; Daily.co → US rooms; Checkr → US) are configured at project setup. (Fly.io was dropped from the inventory by ADR-0019.) A vendor data-flow inventory is maintained as a Privacy Policy appendix.
...
- **Backend on Supabase Edge Functions — Hono "fat" function, supersedes Fly.io + Fastify (2026-06-24, ADR-0019).** Supersedes ADR-0010 §§5–6 and amends §8: one Hono Edge Function (`--no-verify-jwt`, self-verifying), Kysely over Supavisor transaction-mode (`supabase-js` quarantined to the management plane), `pgmq` dropped for deadline-columns + a transactional outbox + a `pg_cron`/`pg_net` `worker-tick`, webhooks terminating on the function (screening webhook defers its Checkr call to the outbox). Reversibility is governed by the **plpgsql canary** (first forced stored-procedure = stop → documented either/or; switch-target is Hono-on-Node) enforced by a `check-no-plpgsql.ts` CI guard, and the pivot is gated on a Supavisor-transaction spike. Domain stays pure TS in `packages/domain`.
